...
if __name__ == "__main__":
    ...
    parser.add_argument("--epochs", type=int, default=106)
    ...
    if args.transform:
        ...
        print("\n####################################")
        with open('predictedTexts.txt', 'w') as f:
            for i, (pred, prob) in enumerate(zip(predicts, probabilities)):
                print("\nProb.  - Predict")
                f.write("Prob.  - Predict\n\n")
        ...
        if args.kaldi_assets:
            ...
            for i, item in enumerate(dtgen.dataset['test']['dt'][:10]):
                ...
                cv2.waitKey(0)
                #print(item)
                print("GT: {0}".format(ground_truth[i]))
                print("Predict: {0}".format(predicts[i], "\n"))
